...
<?php
    ...
    if($consulta){
        $correcta= mysqli_query($link,"UPDATE alumno SET correctas = correctas+1 WHERE id = 1");
        header("Location:examen.php");
    }
    else{
        $incorrecta=mysqli_query($link,"UPDATE alumno SET incorrectas = incorrectas+1 WHERE id = 1");
        header("Location:examen.php");
    }
    mysqli_close($link);
?>   
